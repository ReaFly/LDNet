...
class LDNet_ResNet34(nn.Module):
    ...
    def forward(self, x):
        # x = H*W*3
        bs = x.shape[0]
        e1 = self.encoder1_conv(x)  # H/2*W/2*64
        e1 = self.encoder1_bn(e1)
        e1 = self.encoder1_relu(e1)
        e1_pool = self.maxpool(e1)  # H/4*W/4*64
        e2 = self.encoder2(e1_pool) # H/4*W/4*64
        e3 = self.encoder3(e2)      # H/8*W/8*128
        e4 = self.encoder4(e3)      # H/16*W/16*256
        e5 = self.encoder5(e4)      # H/32*W/32*512
